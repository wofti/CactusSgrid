...
void BNSdataReader(CCTK_ARGUMENTS)
{
  ...
  /* backup lapse and shift if needed */
  {
    if(!set_lapse)
    {
      ...
      printf("BNS_datareader: Lapse (bbb) has been backed up. \n");
    }
  ...
  printf("BNS_datareader: <CCTK_INFO> was called for output \n");
  CCTK_INFO ("Setting up BNS initial data");

  /* say where we are */
  printf("BNS_datareader: level_l=%d  rank=%d \n", level_l, MPIrank);

  /* some info */
  printf("BNS_datareader: sgrid_x_CM = %g\n", sgrid_x_CM);
  ...
  if(!use_interpolator)
  {
    ...
    printf("BNS_datareader: Looking for file %s\n", IDfile);

    /* call interpolator if file cannot be opened */
    fp2 = fopen(IDfile, "rb");
    if(fp2==NULL)
    {
      printf("BNS_datareader: Cannot open %s\n", IDfile);
      ...
      {
        printf("BNS_datareader: Cannot open %s\n", IDfile);
        printf("BNS_datareader: Will call interpolator.\n");
        use_interpolator = 1;
      }
      else
      {
        printf("BNS_datareader: %s exists, no need to call interpolator.\n", IDfile);
        use_interpolator = 0;
  ...
  if(use_interpolator)
  {
    ...
    /* info */
    printf("BNS_datareader: sgridoutdir = %s\n", sgridoutdir);
    printf("BNS_datareader: sgridcheckpoint_indir = %s\n", sgridcheckpoint_indir);
    ...
    printf("BNS_datareader: System call returned: %d\n", ret);
    fflush(stdout);
    if(ret)
    {
      ...
      if(th_lock_curr_til_EOF(fp1)!=0)
        printf("BNS_datareader: Could not lock file %s on proc %d\n",
               sgridallargsfile, MPIrank);
      fprintf(fp1, "%s%s", sgridargs, "\n");
      fclose(fp1);
      /* errorexit("interpolator returned non-zero exit code!"); */
      printf("BNS_datareader: Interpolator returned non-zero exit code: %d\n", ret);
      fflush(stdout);
      // FIXME: need CCTK function that returns number of last level
      if(1) //(level_l == grid->lmax)
      {
        printf("BNS_datareader: FIXME: need CCTK function that returns number of last level\n");
      
        printf("BNS_datareader: Ok I stop here. Sombody has to run sgrid to make ID files "
               "BNS_datareader: for each proc.\n\n");
        printf("BNS_datareader: For proc%d, sgrid needs to be run as follows:\n", MPIrank);
        printf("BNS_datareader: %s --argsfile %s\n\n", sgrid_exe, sgridargsfile);
        printf("BNS_datareader: OR compile sgrid with MPI and run it as:\n");
  ...
  {
    printf("BNS_datareader: Skipping interpolator, reading data from %s\n", IDfile);
  }

  /* info about filenames */
  printf("BNS_datareader: gridfile = %s\n", gridfile);
  printf("BNS_datareader: IDfile = %s\n", IDfile);
  fflush(stdout);
  ...
  for(i=0; i<npoints; i++)
  {
    ...
    if(pr)
    {
      printf("BNS_datareader: (x,y,z)=(%g,%g,%g)\n", x[i],y[i],z[i]);

      printf("BNS_datareader: alpha=%g beta=%g %g %g\n",
	alp[i], betax[i], betay[i], betaz[i]);  

      printf("BNS_datareader: g=%g %g %g %g %g %g \n",
	gxx[i], gxy[i], gxz[i], gyy[i], gyz[i], gzz[i]);
    
      printf("BNS_datareader: k=%g %g %g %g %g %g\n",
	kxx[i], kxy[i], kxz[i], kyy[i], kyz[i], kzz[i]);

      printf("BNS_datareader: rho=%g p=%g epsl=%g v=%g %g %g\n",
        rho[i], press[i], eps[i],
        matter_vx[i], matter_vy[i], matter_vz[i]);

      printf("BNS_datareader: \n");
    }
    ...
    if(detg<=0)
    {
      printf("BNS_datareader: det(g_ij)=%g at ccc=i=%d:  x=%g y=%g z=%g\n",
             detg, i, x[i], y[i], z[i]);
...
void BNSdataPars(CCTK_ARGUMENTS)
{
  ...
  { 
    ...
    if(ret!=0) printf("BNS_datareader: CCTK_ParameterSet returned non-zero exit code!\n");
  }
  ...
  {
    printf("BNS_pars: Only 1 entry in n_list  ->  single polytrope\n");
    printf("BNS_pars: rho0_list %s\n", strrho0);
  }
  else
  {
    printf("BNS_pars: pwp with %d pieces\n", BNS_EoS_n_pieces);
  ...
  /* info */
  printf("BNS_pars: assuming q:=h-1\n");

  printf("BNS_pars: Use pwp with %d pieces \n", BNS_EoS_n_pieces);
  printf("BNS_pars: rho0            q           P           kappa          n           k \n");
  for(i=0;i<=BNS_EoS_n_pieces;i++)
  {
    printf("BNS_pars: %e %e %e %e %e %e \n",
    BNS_EoS_rho0[i], BNS_EoS_q[i], BNS_EoS_P[i], BNS_EoS_kappa[i], BNS_EoS_n[i], BNS_EoS_k[i]);
  }
  printf("BNS_pars: Omega = %g\n", Omega);
  printf("BNS_pars: ecc = %g\n", ecc);
  printf("BNS_pars: rdot = %g\n", rdot);
  printf("BNS_pars: m01 = %g\n", m01);
  printf("BNS_pars: m02 = %g\n", m02);
  printf("BNS_pars: xmax1 - sgrid_x_CM = %g\n", xmax1);
  printf("BNS_pars: xmax2 - sgrid_x_CM = %g\n", xmax2);
  printf("BNS_pars: sgrid_x_CM = %g\n", sgrid_x_CM);
