...
void BNSdataReader(CCTK_ARGUMENTS)
{
  ...
  /* backup lapse and shift if needed */
  {
    if(!set_lapse)
    {
      ...
      printf("BNS_datareader: Lapse  has been backed up. \n");
    }
  ...
  if(use_interpolator)
  {
    ...
    printf("BNS_datareader: System call:\n"); // %s\n", call_interpolator);
    fflush(stdout);
...
void BNSdataPars(CCTK_ARGUMENTS)
{
  ...
  {
    printf("BNS_datareader: Only 1 entry in n_list  ->  single polytrope\n");
    printf("BNS_datareader: rho0_list %s\n", strrho0);
  }
  else
  {
    printf("BNS_datareader: pwp with %d pieces\n", BNS_EoS_n_pieces);
